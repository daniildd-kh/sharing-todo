...
export const fetchLogin = createAsyncThunk('auth/login', async (credentials: {email:string, password:string}) =>{
  try{
...
export const fetchRegistration = createAsyncThunk('auth/register', async (credentials: {name: string, email:string, password:string}) =>{
  try{
...
export const checkAuth = createAsyncThunk('auth/check', async () =>{
  try{
    const response = await $api.get<AuthResponse>('api/auth/refresh-token', {withCredentials: true});
